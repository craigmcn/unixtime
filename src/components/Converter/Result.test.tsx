...
describe("Result", () => {
  ...
  it("renders the UTC date", () => {
    ...
    expect(container.querySelector("#results-utc")).toHaveTextContent("2023-11-14");
  });
  ...
  it("renders an error alert when error is set", () => {
    render(
      <Result
        data={{ ...baseData, error: "Invalid time provided. Switched to current time." }}
      />,
    );
  ...
  it("renders a warning alert when warning is set", () => {
    render(
      <Result
        data={{ ...baseData, warning: "Invalid timezone provided. Switched to UTC." }}
      />,
    );
  ...
  it("does not show a timezone row for UTC", () => {
    ...
    expect(container.querySelector("#results-timezone")).not.toBeInTheDocument();
  });
