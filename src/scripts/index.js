...
converter.addEventListener('submit', e => {
            ...
            if (timezone !== 'UTC') {
                ...
                if (!time.match(nowRegex)) {
                    momentDate = moment.unix(
                        momentDate.unix() + timezoneOffset * 60
                    )
                }
            }
        } else {
    ...
    if (error) {
        ...
        document.getElementById('results-timestamp').innerHTML =
            momentDate.unix()
        document.getElementById('results-utc').innerHTML = `${momentDate
            .utc()
            .format('dddd, MMMM D YYYY h:mm:ss a z')}<br>${momentDate
            .utc()
            .format('YYYY-MM-DD HH:mm:ss z')}`
        document.getElementById('results-timezone-name').innerHTML =
            moment.tz.zone(timezone) ? moment.tz.zone(timezone).name : 'UTC'
        ...
        if (timezone === 'UTC') {
            ...
            document.getElementById('results-timezone-name').style.display =
                document.getElementById('results-timezone').style.display =
                    'none'
        } else {
            documentTitle = momentDate
                .tz(timezone)
        ...
        document.getElementById('results-iso8601').innerHTML =
            momentDate.format('YYYY-MM-DDTHH:mm:ssZZ')
        document.getElementById('results-rfc2822').innerHTML =
            momentDate.format('ddd, DD MMM YYYY HH:mm:ss ZZ')
        document
