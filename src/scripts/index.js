...
converter.addEventListener('submit', (e) => {
    ...
    if (error) {
        ...
        document.getElementById(
            ...
            )}<br>${momentDate.utc().format('YYYY-MM-DD HH:mm:ss z')}`
        document.getElementById(
            'results-timezone-name'
        ).innerHTML = moment.tz.zone(timezone)
        ...
        document.getElementById(
            'results-iso8601'
        ).innerHTML = momentDate.format('YYYY-MM-DDTHH:mm:ssZZ')
        document.getElementById(
            'results-rfc2822'
        ).innerHTML = momentDate.format('ddd, DD MMM YYYY HH:mm:ss ZZ')
        document
            .getElementById('results-links-web')
            .setAttribute(
                'href',
                `https://www.craigmcn.com/unixtime/?time=${time}&timezone=${
                    timezone || ''
                }`
            )
    }
